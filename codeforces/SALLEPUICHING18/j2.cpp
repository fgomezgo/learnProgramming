...
long long querys(int nodo, int ini, int fin, int b, int e){
    //return 0LL;
        cout<<"querryyy"<<endl;
        cout<<nodo<<", "<<ini<<" "<<fin<<" "<<b<<" "<<e<<endl;
    ...
    if(ini == b && fin == e)
        return tree[nodo];
    int mid = (ini+fin)>>1;
    return querys(nodo<<1,ini,mid,b,min(mid,e))+querys(nodo<<1|1,mid+1,fin,max(b,mid+1),e);
}
...
long long dist(int u, int v){
    ...
    while(path[u] != path[v]){
        ...
        cout<<"u v "<<u<<" "<<v<<endl;
        cout<<p_path[path[u]].fst<<endl;
        cout<<"adasds"<<endl;
        cout<<renum[p_path[path[u]].fst];
        cout<<" "<<renum[u]-1<<": "<<endl;
        sum += querys(1,1,n,renum[p_path[path[u]].fst],renum[u]-1);
        cout<<querys(1,1,n,renum[p_path[u].fst],renum[u]-1)<<endl;
        sum += p_path[path[u]].snd;
        u = parent[p_path[u].fst].fst;
    }
    if(u > v)
        swap(u,v);
    sum += querys(1,1,n,renum[u],renum[v]-1);
    return sum;
}   
...
int main(){
    ...
    cout<<"renum"<<endl;
    for(int i = 1; i <= n; ++i) cout<<renum[i]<<" "; cout<<endl;
    cout<<"path"<<endl;
    for(int i = 1; i <= n; ++i) cout<<path[i]<<" "; cout<<endl;
    cout<<"P_path"<<endl;
    for(int i = 1; i < pos; ++i) cout<<p_path[i].fst<<" "; cout<<endl;
    for(int i = 1; i < pos; ++i) cout<<p_path[i].snd<<" "; cout<<endl;
    int Q;
    scanf("%d", &Q);
    int t;
    while(Q--){
